...
/* ============================================================
 * BLE passive scanner — counts unique devices over a 2s window
 * and prints `BLE_COUNT,<ms>,<unique>,<total_ads>` on UART.
 * Runs concurrently with Wi-Fi CSI via SW coex.
 * ============================================================ */
#define BLE_MAX_UNIQUE 256
#define BLE_REPORT_MS  2000

static uint8_t  s_ble_macs[BLE_MAX_UNIQUE][6];
static int      s_ble_unique = 0;
static uint32_t s_ble_total  = 0;
...
static int ble_gap_event_cb(struct ble_gap_event *event, void *arg)
{
    ...
    bool seen = false;
    for (int i = 0; i < s_ble_unique; i++) {
        if (memcmp(s_ble_macs[i], addr, 6) == 0) { seen = true; break; }
    }
    if (!seen && s_ble_unique < BLE_MAX_UNIQUE) {
        memcpy(s_ble_macs[s_ble_unique++], addr, 6);
    }
...
static void ble_on_sync(void)
{
    struct ble_gap_disc_params disc_params = {
        .itvl           = 0x0060,   /* 60ms scan interval */
        .window         = 0x0030,   /* 30ms scan window (50% duty) */
        .filter_policy  = 0,
        .limited        = 0,
        .passive        = 1,        /* passive scan — no scan-req frames */
        .filter_duplicates = 0,     /* we want ALL ads to count totals */
    };
    int rc = ble_gap_disc(BLE_OWN_ADDR_PUBLIC, BLE_HS_FOREVER,
                          &disc_params, ble_gap_event_cb, NULL);
    if (rc != 0) {
        ets_printf("BLE_ERROR,disc_start,%d\n", rc);
    } else {
        ets_printf("BLE_INFO,scanner_started\n");
    }
}
...
static void ble_report_task(void *param)
{
    ...
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(BLE_REPORT_MS));
        elapsed_ms += BLE_REPORT_MS;
        xSemaphoreTake(s_ble_mutex, portMAX_DELAY);
        int      unique = s_ble_unique;
        uint32_t total  = s_ble_total;
        s_ble_unique = 0;     /* reset window */
        s_ble_total  = 0;
        xSemaphoreGive(s_ble_mutex);
        ets_printf("BLE_COUNT,%u,%d,%u\n",
                   (unsigned)elapsed_ms, unique, (unsigned)total);
    }
}
...
static void ble_scan_init(void)
{
    ...
    if (ret != ESP_OK) {
        ets_printf("BLE_ERROR,port_init,%d\n", ret);
        return;
    }
    ble_hs_cfg.sync_cb = ble_on_sync;
    nimble_port_freertos_init(ble_host_task);
    xTaskCreate(ble_report_task, "ble_report", 4096, NULL, 5, NULL);
}
...
void app_main()
{
    ...
    /**
     * @brief Initialize BLE passive scanner (runs alongside Wi-Fi via coex)
     */
    ble_scan_init();
}
